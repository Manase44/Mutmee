...
const UpdateProfile = () => {
    ...
    const uploadImage = async () => {
      ...
      const postUserUpdateDetails = async(data) => {
        setLoading(true)
        try {
            const response = await axios.patch(`${server_url}/user/profile`, data, {withCredentials: true})
    ...
    return (
        <form onSubmit={userDetailsUpdateForm.handleSubmit} className="selected-setting-container">
            <>
                ...
                {error ? <p className="edit-photo-selected-image">{error}</p> : imageInput && <span className="edit-photo-selected-image">{imageInput.name}</span>}
            </>
            ...
            <button type="submit">{loading ? <span className="updating-button">
                <Oval
                    visible={true}
                    height="20"
                    width="20"
